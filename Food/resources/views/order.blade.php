     ...
     <div class="container">
         ...
         <div class="row">
                     ...
                     <div class="order-cart-item d-flex justify-content-around">
                         ...
                         <div class="food-desc-wrapper">
                             ...
                             <div class="mobile d-flex pt-2">
                                 <p class="price">{{ number_format($orderItem->menu->price, 0) }} VND</p>
                                 <p class="quantity">x{{ $orderItem->quantity }}</p>
                                 <p class="cart-item-total">{{ number_format($orderItem->menu->price * $orderItem->quantity, 0) }} VND</p>        
                             </div>
                             <p class="text-muted desktop">{{ $orderItem->menu->description }}</p>
                         </div>
                         <p class="price desktop">{{ number_format($orderItem->menu->price, 0) }} VND</p>
                         <p class="quantity desktop">x{{ $orderItem->quantity }}</p>
                         <p class="cart-item-total desktop">{{ number_format($orderItem->menu->price * $orderItem->quantity, 0) }} VND</p>
                     </div>
                     <hr>
                 @endforeach
                 </div>
             ...
             <div class="order-summary p-4 col-lg-3 offset-lg-1 col-md-12">
                 ...
                 <div class="d-flex justify-content-between">
                     <h6>Subtotal</h6>
                     <p>{{ number_format($subtotal = $activeOrder->getSubtotal(),0) }} VND</p>
                 </div>
                 <div class="d-flex justify-content-between">
                     <h6>Discount</h6>
                     <p>-{{ number_format($discount = $activeOrder->getDiscount($subtotal),0) }} VND</p>
                 </div>
                 <div class="d-flex justify-content-between">
                     <h6>Tax (6%)</h6>
                     <p>{{ number_format($tax = $activeOrder->getTax($subtotal, $discount),0) }} VND</p>
                 </div>
                 <hr>
                 <div class="d-flex justify-content-between">
                     <h6>Total</h6>
                     <p>{{ number_format($activeOrder->getTotal($subtotal, $discount, $tax),0) }} VND</p>
                 </div>
             </div>
         </div>
     </div>
         ...
         <table class="table table-hover">
             ...
             <tbody>
                 ...
                 @endif
                     ...
                     <td>{{ number_format($order->getTotalFromScratch(),0) }} VND</td>
                     <td>
                         @if ($order->completed)
                             <div class="px-3 alert alert-success">
                                 Fulfilled
