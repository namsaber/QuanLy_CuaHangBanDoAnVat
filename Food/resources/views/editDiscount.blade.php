<!-- 
 ...
 <section class="min-vh-100 flex-center py-5">
     <div class="container">
         ...
         <div class="mb-3">
             <label for="minSpend" class="form-label">Minimum Spend (VND)</label>
             <input type="number" class="form-control @error('minSpend') is-invalid @enderror" id="minSpend" 
                 name="minSpend" value="{{ old('minSpend') ? old('minSpend') : $discount->minSpend }}">
             @error('minSpend')
                 <span class="invalid-feedback" role="alert">
                     <strong>{{ $message }}</strong>
                 </span>
         ...
         <div class="mb-3">
             <label for="cap" class="form-label">Cap At (VND)</label>
             <input type="number" class="form-control @error('cap') is-invalid @enderror" id="cap" name="cap" 
                 min="0" value="{{ old('cap') ?  old('cap') : $discount->cap }}">
             @error('cap')
                 <span class="invalid-feedback" role="alert">
                     <strong>{{ $message }}</strong>
                 </span>
